...
const float vel_kp     = 25.0; //5
const float vel_ki     = 50.0; //10
const float vel_kd     = 0.0; //0
const float vel_kp_ABS = 100.0; //100
const float vel_ki_ABS = 100.0; //100
const float pos_kp     = 50.0; //10
const float pos_kd     = 5.0; //5
const float pos_ki     = 50.0; //10
const float pos_ei_sat = 1.0; 
...
const double tick2m    = 3*0.00000323766; // To confirm
...
void cmdCallback ( const geometry_msgs::Twist&  twistMsg ){
  ...
  pourcentage_decel = twistMsg.linear.y; // %
...
void ctl(){
  ...
  float alpha   = time_period_low / (filter_rc + time_period_low);
  float vel_fil = alpha * vel_raw + ( 1 - alpha) * vel_old;    // Filter
  ...
  //////////////////////////////////////////////////////
    ...
    vel_error_int = vel_error_int + vel_error * time_period_low * 0.001;
  ...
  ///////////////////////////////////////////////////////
    ...
    pos_error_ddt =  0.0    - vel_fil;
    pos_error_int = pos_error_int + pos_error * time_period_low * 0.001;
  ...
  ///////////////////////////////////////////////////////
    ...
    // Low-level Velocity control to simulate braking with ABS
    // Commands received in [m/sec] setpoints
    
    float vel_ref, vel_error vel_initial vel_final;
    
    temps_decel = temps_decel+0.002;
    vel_ref       = dri_ref-temps_decel*pourcentage_decel*5; //* a modif decel_max; 

    vel_error     = vel_ref - vel_fil;
    vel_error_int = vel_error_int + vel_error * time_period_low * 0.001;
    dri_cmd       = vel_kp_ABS * vel_error + vel_ki_ABS * vel_error_int;
